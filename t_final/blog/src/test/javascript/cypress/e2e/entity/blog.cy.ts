...
describe('Blog e2e test', () => {
  ...
  const blogSample = { name: 'infantile coaxingly batting', handle: 'oh besides' };
  ...
  describe('new Blog page', () => {
    ...
    it('should create an instance of Blog', () => {
      cy.get(`[data-cy="name"]`).type('digress usefully regarding');
      cy.get(`[data-cy="name"]`).should('have.value', 'digress usefully regarding');

      cy.get(`[data-cy="handle"]`).type('um afore');
      cy.get(`[data-cy="handle"]`).should('have.value', 'um afore');
